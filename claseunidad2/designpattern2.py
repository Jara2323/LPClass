...
m= Movie("Avengers: Infinity War")
m = showSubtitlesDecorator(m)
m= showAudioDecorator(m)
print(m.showMovieInfo())

# patron Command: patron de comportamiento que encapsula una serie de comportamientos o acciones, en un objeto y los trata como tales 
# encapsulamiento de tareas, rastreo de historico de operaciones (deshacer y rehacer), relacionado con el concepto de callback

#responde a tres preguntas: 
# que clases estan implicadas?
# que rol cumple cada clase? 
# cual es la relacion entre esas clases?

# invocador -> comand -> receptor 
...
class catalog: 
    def __init__(this):
        this.movies = []
    def add_movie(this, movie):
        this.movies.append
        print(f"peliculas en el catalogo: {this.movies}")

class addMovieComand(IComand):
    def __init__(this, catalog, movie):
     this.catalog = catalog
     this.movie = movie

    def execute(this):
        this.catalog.add_movie(this.movie)
        
#invocador:       
class moviesRemoteControl:
    def __init__(this: IComand):
        this.command = IComand
        this.history = []
...
myCatalog = catalog()
cmd = addMovieComand(myCatalog, "Interestellar")
invoker = moviesRemoteControl()
invoker.execute(cmd)

#patron facade: proporciona una clase general o una abstraccion como punto de acceso comun
# cliente -> facade -> subsistemas 1
#                   -> subsistemas 2
#                   -> subsistemas 3

class authSystem:
    def __init__(this):
        validUsers = [{"usr": "diego", "password": "1234"}, {"user": "maria", "password": "abcd"}]
    
    def authenticate(this, usr, password):
        #buscar en la lista de usuarios valido
        pass
    def addUser(this, usr, password):
        #añadir ala lista de usuarios validos
        pass

class reportingSystem:
    def generateReport(this, user):
        return f"reporte generado para {user}"

class recomendationSystem:
    def recommend(this, user):
        return f"lista de recomendaciones para {user}"

class platformFacade:
    def __init__(this):
        this.auth = authSystem()
        this.report = reportingSystem()
        this.recommend = recomendationSystem()
    
    def mainPanel(this, usr, password):
        #mostrar opciones para ejecutar operaciones de los subsistemas con los que se puede interactuar
        pass
 
#cliente    
mainSystem = platformFacade() #interfaz principal con el cliente de todo el sistema
